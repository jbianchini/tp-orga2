...
# Crear y lanzar hilos para jugadores por cofre
threads = []
for i in range(5):
...
for i in range(4):
    t = threading.Thread(target=player_action, args=(f"C-Jugador-{i+1}", chest_c))
    threads.append(t)
    t.start()
...
print(f"programa terminado en {elapsed:.3f} segundos")
